"""Seed the bankrupt-ex story with existing local character assets.

Copies the user's approved character key art from D:\\AI剧\\角色人物图 into the
story's generated assets folder and marks the image / S1-portrait jobs as
successful, so the app does not resubmit paid Vidu image tasks. It never calls
the Vidu API.
"""
...
def main() -> int:
    ...
    if not isinstance(source_assets, dict):
        raise RuntimeError("bankrupt-ex source_assets missing")
